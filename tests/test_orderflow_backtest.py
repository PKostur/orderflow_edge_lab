...
class OrderFlowBacktestTests(unittest.TestCase):
    ...
    @staticmethod
    def _trade(symbol, ts, *, bid, ask, buy, sell, imbalance, micro):
        return {
            "event_type": "trade", "symbol": symbol, "exchange_ts_ms": ts,
    ...
    def test_crossing_spread_and_fees_are_charged(self):
        rows = [
            self._trade("BTC_USDT", 900, bid=100, ask=101, buy=9, sell=1, imbalance=.5, micro=100.8),
            self._trade("ENA_USDT", 1000, bid=10.00, ask=10.01, buy=9, sell=1, imbalance=.6, micro=10.008),
            self._trade("ENA_USDT", 2000, bid=10.03, ask=10.04, buy=9, sell=1, imbalance=.6, micro=10.038),
        ]
    ...
    def test_btc_confirmation_blocks_opposite_context(self):
        rows = [
            self._trade("BTC_USDT", 900, bid=100, ask=101, buy=1, sell=9, imbalance=-.5, micro=100.2),
            self._trade("ENA_USDT", 1000, bid=10.00, ask=10.01, buy=9, sell=1, imbalance=.6, micro=10.008),
            self._trade("ENA_USDT", 2000, bid=10.03, ask=10.04, buy=9, sell=1, imbalance=.6, micro=10.038),
        ]
    ...
    def test_future_context_does_not_leak_backward(self):
        rows = [
            self._trade("ENA_USDT", 1000, bid=10.00, ask=10.01, buy=9, sell=1, imbalance=.6, micro=10.008),
            self._trade("BTC_USDT", 1500, bid=100, ask=101, buy=9, sell=1, imbalance=.5, micro=100.8),
            self._trade("ENA_USDT", 2000, bid=10.03, ask=10.04, buy=9, sell=1, imbalance=.6, micro=10.038),
        ]
        report = evaluate(self._write(rows), BacktestConfig(horizons_ms=(1000,), fee_bps_round_trip=(0.0,), cooldown_ms=0))
        first = [x for x in report["observations"] if x["signal_ts_ms"] == 1000 and x["family"] == "aligned"]
