...
class HoldoutBindingTests(unittest.TestCase):
    ...
    def test_promotion_binding_matches_exact_observations_and_registry(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            source, source_sha, registry_path, freeze_path, freeze = self.fixture(root)
            observations = self.write_observations(root, source_sha)
            ...
            assessment, reasons = assess_bound_promotion(
                report_path, "P1", economics_path, freeze_path, holdout_path
            )
