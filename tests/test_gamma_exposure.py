...
def test_black_scholes_gamma_positive():
    assert black_scholes_gamma(100.0, 100.0, 0.5, 0.5, 0.01) > 0


def test_snapshot_is_signed_proxy_and_manifest_verifies():
    ...
    assert snapshot["metrics"]["gross_gamma_exposure_usd_per_1pct"] > 0
    assert snapshot["metrics"]["call_gamma_exposure_usd_per_1pct"] > 0
    assert snapshot["metrics"]["put_gamma_exposure_usd_per_1pct"] > 0
    assert snapshot["limitations"]["dealer_positioning_observed"] is False
    assert snapshot["claims"]["live_order_transmission_supported"] is False
    assert verify_gamma_snapshot(snapshot)
    tampered = deepcopy(snapshot)
    tampered["metrics"]["reference_spot_usd"] += 1
    assert not verify_gamma_snapshot(tampered)


def test_history_waits_for_predeclared_sample_guards():
    instruments, summaries = fixtures()
    snapshot = build_gamma_snapshot(instruments, summaries, PROTOCOL, observed_at_ms=1_790_000_000_000)
    ena, trend, xs = reports()
    history = append_comparison_history(snapshot, ena, trend, xs, PROTOCOL)
    assert history["status"] == "collecting_gamma_history"
    assert history["guards"]["observed_forward_snapshots"] == 1
    assert history["claims"]["strategy_parameters_changed"] is False


def test_history_can_reach_descriptive_ready_without_parameter_selection():
    instruments, summaries = fixtures()
    ...
    assert history["guards"]["observed_forward_snapshots"] == 12
    assert history["guards"]["observed_completed_outcome_increments"] >= 5
    assert history["status"] == "descriptive_comparison_ready"
