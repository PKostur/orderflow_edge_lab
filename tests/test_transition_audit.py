...
class TransitionAuditTests(unittest.TestCase):
    def test_valid_submit_open_close_sequence_passes_transition_audit(self):
        initial = _state()
        ...
        self.assertEqual(audit_checkpoint_transitions(records), 3)

    def test_rejects_equity_change_outside_close_event(self):
        initial = _state()
        corrupt = deepcopy(initial)
        corrupt["equity"] = 11_000.0

        with self.assertRaisesRegex(StateCorruptionError, "equity changed outside a close"):
            audit_checkpoint_transitions([
                _row("engine_initialized", {}, initial),
                _row("approval_token_rejected", {"intent_id": "abc"}, corrupt),
            ])

    def test_rejects_kill_switch_that_does_not_account_for_all_pending_intents(self):
        initial = _state()
        initial["pending"] = {"a": {}, "b": {}}
        ...
        with self.assertRaisesRegex(StateCorruptionError, "cancellation set"):
            audit_checkpoint_transitions([
                _row("engine_initialized", {}, initial),
                _row("kill_switch_engaged", {"cancelled_intents": ["a"]}, killed),
            ])

    def test_rejects_close_when_equity_delta_disagrees_with_reported_pnl(self):
        initial = _state()
        ...
        with self.assertRaisesRegex(StateCorruptionError, "equity delta"):
            audit_checkpoint_transitions([
                _row("engine_initialized", {}, initial),
