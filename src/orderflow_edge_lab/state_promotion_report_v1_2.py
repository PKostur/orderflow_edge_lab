...
class StatePromotionReportV12Error(ValueError):
    pass
...
def build_state_promotion_report_v1_2(aggregate: Mapping[str, Any]) -> dict[str, Any]:
    if aggregate.get("experiment") != "regime_research_v1_2_market_state_screen":
        raise StatePromotionReportV12Error("input is not a regime-research-v1.2 market-state screen")
    ...
    for association in aggregate.get("association_summary", []):
        ...
        if incremental_row is not None:
            incremental_passed = bool(incremental_row.get("v1_2_eligible_incremental_feature"))

        state_screen_passed = bool(association.get("v1_2_eligible_state_association"))
        conditioning_candidate = bool(state_screen_passed and is_representative)
        rows.append(
            {
                "research_family": family,
                "feature": feature,
                "target": target,
                "independent_dependence_clusters": int(association.get("independent_batches") or 0),
                ...
                "effect_size_floor_passed": bool(association.get("v1_2_effect_size_floor_passed")),
                ...
                "eligible_for_strategy_conditioning_v1_1_freeze": conditioning_candidate,
            }
        )

    rows.sort(key=lambda row: (row["research_family"], row["feature"], row["target"]))
    eligible = [row for row in rows if row["eligible_for_strategy_conditioning_v1_1_freeze"]]
    readiness = aggregate.get("v1_2_readiness", {})
    dependence_audit = aggregate.get("dependence_audit", {})
    return {
        "schema_version": 1,
        "experiment": "state_promotion_report_v1_2",
        "source_protocol": aggregate.get("protocol_name"),
        "source_status": aggregate.get("status"),
        "evidence_start_utc": aggregate.get("evidence_start_utc"),
        "evidence_start_batch_id": aggregate.get("evidence_start_batch_id"),
        "independent_dependence_cluster_count": int(
            dependence_audit.get("independent_cluster_count") or aggregate.get("independent_batch_count") or 0
        ),
        "collapsed_report_count": int(dependence_audit.get("collapsed_report_count") or 0),
        "minimum_independent_dependence_clusters": readiness.get("minimum_independent_batches"),
        "screen_ready": bool(readiness.get("enough_forward_dependence_clusters")),
        "candidate_count": len(eligible),
        "candidates": rows,
        "selection_rule": (
            "lexical reporting only; no strategy PnL, PF, expectancy, return, or association-ranked competition is used for selection"
        ),
        "claims": {
            "market_state_first": True,
            "strategy_pnl_used": False,
            "pnl_fields_present": False,
            "dependence_clusters_counted_instead_of_raw_capture_ids": True,
            "redundancy_representative_required_for_strategy_conditioning_v1_1": True,
