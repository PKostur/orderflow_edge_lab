...
def main():
    ...
    try:
        ...
        if output.resolve() in {Path(args.registry).resolve(), Path(args.observations).resolve()}:
            raise ValueError("output must not overwrite inputs")
        report = build_validation_report(args.registry, args.observations, observed_through=args.observed_through)
